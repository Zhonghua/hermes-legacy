...
int main(int argc, char* argv[])
{
  ...
  EssentialBCs bcs_T(&temp_reactor);

  // Solutions.
  Solution T_coarse, w_coarse;

  // Create H1 spaces with default shapesets.
  H1Space T_space(&T_mesh, &bcs_T, P_INIT);
  H1Space w_space(MULTI ? &w_mesh : &T_mesh, P_INIT);

  // Define constant initial conditions.
  info("Setting initial conditions.");
  Solution T_time_prev(&T_mesh, T_INITIAL);
  Solution w_time_prev(&w_mesh, W_INITIAL);
  Solution T_time_new(&T_mesh);
  Solution w_time_new(&w_mesh);
  ...
  while (current_time < SIMULATION_TIME)
  {
    ...
    if (current_time <= REACTOR_START_TIME) {
      ...
      Space::update_essential_bc_values(Hermes::vector<Space*>(&T_space, &w_space), current_time);
    }
    ...
    do
    {
      ...
      Hermes::vector<Space *>* ref_spaces 
          = Space::construct_refined_spaces(Hermes::vector<Space *>(&T_space, &w_space));
      ...
      bool verbose = true;
      if (runge_kutta.rk_time_step(current_time, time_step, 
                                   Hermes::vector<Solution *>(&T_time_prev, &w_time_prev), 
                                   Hermes::vector<Solution *>(&T_time_new, &w_time_new), 
      ...
      OGProjection::project_global(Hermes::vector<Space *>(&T_space, &w_space), 
          Hermes::vector<Solution *>(&T_time_new, &w_time_new), 
          Hermes::vector<Solution *>(&T_coarse, &w_coarse),
          matrix_solver); 
      ...
      double err_est_rel_total = adaptivity->calc_err_est(Hermes::vector<Solution *>(&T_coarse, &w_coarse), 
                                 Hermes::vector<Solution *>(&T_time_new, &w_time_new)) * 100;

      // Report results.
      info("ndof_coarse: %d, ndof_fine: %d, err_est_rel: %g%%", 
	    Space::get_num_dofs(Hermes::vector<Space *>(&T_space, &w_space)), 
      ...
      else 
      {
        ...
        if (Space::get_num_dofs(Hermes::vector<Space *>(&T_space, &w_space)) >= NDOF_STOP) 
          done = true;
        else
